...
client.on("interactionCreate", async (interaction) => {
  ...
  if (
    ...
    try {
      if (!queue.connection) {
        await queue.connect(interaction.member.voice.channel).then(() => {
          const ult = createAudioResource(join(__dirname, "/audio/ult.mp3"), {
            inlineVolume: true,
          });
          ult.volume.setVolume(1);
          queue.connection.playStream(ult);
        });
      }
      // HERE IS ENTER SOUND
    } catch {
    ...
    await sleeping();
    console.log("finished 2 second wait");
    // const resource = createAudioResource(join(__dirname, "/audio/ult.mp3"), {
    //   inlineVolume: true,
    // });
